...
export function getMetadataArgsStorage(): MetadataArgsStorage {
    ...
    // another reason is that when we run migrations ferret is being called from a global package
    // and it may load entities which register decorators in ferret of local package
    // this leads to impossibility of usage of entities in migrations and cli related operations
    const globalScope = PlatformTools.getGlobalVariable();
    if (!globalScope.ferretMetadataArgsStorage)
        globalScope.ferretMetadataArgsStorage = new MetadataArgsStorage();

    return globalScope.ferretMetadataArgsStorage;
}

/**
 * Reads connection options stored in ferret.config configuration file.
 */
 ...
 * Only one connection from ferret.config will be created (name "default" or connection without name).
 */
export async function createConnection(): Promise<Connection>;

/**
 * Creates a new connection from the ferret.config file with a given name.
 */
 ...
 * based on content of ferret.config (json/js/yml/xml/env) file or environment variables.
 * Only one connection from ferret.config will be created (name "default" or connection without name).
 */
 ...
 * If connection options were not specified, then it will try to create connection automatically,
 * based on content of ferret.config (json/js/yml/xml/env) file or environment variables.
 * All connections from the ferret.config will be created.
 */
